...
def _to_rel_posix(base_dir: Path, path: Path) -> str:
    return os.path.relpath(path, base_dir).replace("\\", "/")


def _run_and_stream(command: list[str], cwd: Path) -> int:
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="config-prompt-image-swap.txt",
        help="Config file path (default: config-prompt-image-swap.txt)",
    )
    parser.add_argument(
        "--model",
        default="",
    ...
    config_path = _resolve_path(base_dir, args.config, "config-prompt-image-swap.txt")
    config = _read_kv_config(config_path)
    if not config:
        raise RuntimeError(f"Config is empty or missing: {config_path}")

    image_edit_swap_path = (base_dir / "image-edit-swap.py").resolve()
    if not image_edit_swap_path.exists():
        raise RuntimeError(f"Missing script: {image_edit_swap_path}")
    ...
    run_config["Prompt"] = _to_rel_posix(base_dir, generated_paths[0])
    run_config["FileNumbers"] = str(prompt_number)
    if not (run_config.get("Model") or "").strip():
        run_config["Model"] = "true"

    default_role_data_path = (base_dir / "config-image-edit-swap.json").resolve()
    if default_role_data_path.exists() and not (run_config.get("RoleData") or "").strip():
        run_config["RoleData"] = _to_rel_posix(base_dir, default_role_data_path)
    ...
    try:
        with tempfile.NamedTemporaryFile(
            mode="w",
            encoding="utf-8",
            suffix=".prompt-image-swap.config",
            delete=False,
            dir=str(base_dir),
        ) as tf:
        ...
        print("Starting image-edit-swap.py")
        return _run_and_stream(
            [sys.executable, str(image_edit_swap_path), "--config", str(temp_config_path)],
            cwd=base_dir,
        )
    finally:
